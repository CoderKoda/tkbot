...
function getUserData(groupId, userId) {
  return ensureUser(loadDB(), groupId, userId);
}
...
function checkCooldown(user, key) {
  ...
  if (!wait || !last || Date.now() - last >= wait) return { ok: true };
...
function getTop(groupId, limit = 10) {
  ...
  return Object.entries(group).map(([jid, raw]) => [jid, migrateUser(raw)]).filter(([, user]) => totalWealth(user) > 0).sort((a, b) => totalWealth(b[1]) - totalWealth(a[1])).slice(0, limit);
}
