...
const economyCommands = [
  makeEcoCmd({
    ...
    async execute(sock, msg, args, extra) {
      ...
      if (!result.ok) return extra.reply(`⏳ Daily already claimed. Try again in *${formatWait(result.waitMs)}*.`);
      return extra.reply(`💰 *Daily reward claimed!*\n\n+${fmtCoins(result.amount)} coins\n\n${walletLine(result.user)}${levelNotice(result.levelResult)}`);
    },
  }),
  ...
  makeEcoCmd({
    ...
    async execute(sock, msg, args, extra) {
      ...
      if (!result.ok) return extra.reply(`⏳ Weekly already claimed. Try again in *${formatWait(result.waitMs)}*.`);
      return extra.reply(`📅 *Weekly reward claimed!*\n\n+${fmtCoins(result.amount)} coins\n\n${walletLine(result.user)}${levelNotice(result.levelResult)}`);
    },
  }),
  ...
  makeEcoCmd({
    ...
    async execute(sock, msg, args, extra) {
      ...
      if (!result.ok) return extra.reply(`⏳ You're tired. Work again in *${formatWait(result.waitMs)}*.`);
      return extra.reply(`💼 *Work complete!*\n\nYou earned *${fmtCoins(result.amount)}* coins.\n\n${walletLine(result.user)}${levelNotice(result.levelResult)}`);
    },
  }),
  ...
  makeEcoCmd({
    name: 'crime',
    aliases: ['rob', 'heist'],
    description: 'Risk a chosen amount of coins on crime (10m cooldown)',
    usage: ',crime <wager>',
    async execute(sock, msg, args, extra) {
      const wager = Number(args[0]);
      if (!Number.isInteger(wager) || wager < 1) {
        return extra.reply('❌ Usage: `,crime <wager>`\nExample: `,crime 500`');
      }

      const result = doCrime(extra.from, extra.sender, wager);
      if (!result.ok) {
        if (result.error === 'funds') {
          return extra.reply(`❌ Wager rejected. You only have *${fmtCoins(result.balance)}* 🪙, but you tried to risk *${fmtCoins(result.wager)}* 🪙.`);
        }
        if (result.error === 'invalid_wager') {
          return extra.reply('❌ Wager must be a positive whole number.');
        }
  ...
  makeEcoCmd({
    ...
    async execute(sock, msg, args, extra) {
      ...
      if (!result.ok) return extra.reply(`⏳ Rest your pickaxe. Mine again in *${formatWait(result.waitMs)}*.`);
  ...
  makeEcoCmd({
    name: 'dep', aliases: ['deposit'], description: 'Deposit coins into bank', usage: ',dep <amount>',
    async execute(sock, msg, args, extra) {
      const amount = parseAmount(args); if (!amount) return extra.reply('❌ Usage: `,dep <amount>`');
      const result = deposit(extra.from, extra.sender, amount);
      if (!result.ok) return extra.reply(result.error === 'funds' ? '❌ Not enough coins in wallet.' : '❌ Invalid amount.');
      return extra.reply(`🏦 *Deposited ${fmtCoins(result.amount)} coins*\n\n${walletLine(result.user)}`);
    },
  }),

  makeEcoCmd({
    name: 'wd', aliases: ['withdraw'], description: 'Withdraw coins from bank', usage: ',wd <amount>',
    async execute(sock, msg, args, extra) {
      const amount = parseAmount(args); if (!amount) return extra.reply('❌ Usage: `,wd <amount>`');
      const result = withdraw(extra.from, extra.sender, amount);
      if (!result.ok) return extra.reply(result.error === 'funds' ? '❌ Not enough coins in bank.' : '❌ Invalid amount.');
      return extra.reply(`💵 *Withdrew ${fmtCoins(result.amount)} coins*\n\n${walletLine(result.user)}`);
    },
  }),

  makeEcoCmd({
    name: 'pay', aliases: ['transfer', 'give'], description: 'Send coins to another member', usage: ',pay @user <amount>',
    async execute(sock, msg, args, extra) {
      const targetId = getMentionTarget(msg); const amount = parseAmount(args);
      ...
      if (!result.ok) return extra.reply(result.error === 'funds' ? `❌ Not enough coins. Wallet: *${fmtCoins(getUserData(extra.from, extra.sender).wallet)}* 🪙` : '❌ Invalid transfer.');
  ...
  makeEcoCmd({
    name: 'shop', aliases: ['store', 'market'], description: 'View the shop', usage: ',shop',
    async execute(sock, msg, args, extra) {
      const lines = SHOP.map((item) => `• \`${item.id}\` ${item.name} — *${fmtCoins(item.price)}* 🪙`);
      return extra.reply(`🛒 *Shop*\n\n${lines.join('\n')}\n\nBuy with: \,buy <item>\nExample: \,buy pizza`);
    },
  }),

  makeEcoCmd({
    name: 'buy', aliases: ['purchase'], description: 'Buy an item from the shop', usage: ',buy <item>',
    async execute(sock, msg, args, extra) {
      const itemId = args.join(' ').trim().toLowerCase(); if (!itemId) return extra.reply('❌ Usage: `,buy <item>`\nUse `,shop` to see items.');
      const result = buyItem(extra.from, extra.sender, itemId);
      if (!result.ok) return extra.reply(result.error === 'item' ? '❌ Item not found. Use `,shop` to browse.' : result.error === 'funds' ? `❌ Not enough coins. Need *${fmtCoins(result.item.price)}* 🪙` : '❌ Purchase failed.');
      return extra.reply(`🛍️ *Purchased ${result.item.name}!*\n\nPrice: *${fmtCoins(result.item.price)}* 🪙\n\n${walletLine(result.user)}`);
    },
  }),
];
